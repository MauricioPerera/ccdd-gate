...
signature: "def signature_mismatch(source: str, fn_name: str, expected_signature: str, target_line: int = None) -> str"
test_command: "python -m unittest tests.test_sig_check"
test_cwd: "../../.."
budget: { cyclomatic_max: 9, nesting_max: 3, params_max: 4, lines_max: 30 }
deps_allowed: []
...
in:  source: str (código Python), fn_name: str, expected_signature: str (p.ej. "def f(x: int) -> str"),
     target_line: int|None (OPCIONAL: línea de la def a verificar, para desambiguar homónimas)
out: str  ("" si coincide; descripción NO vacía si difiere o la función no está)
compara: nombre de la def implementada == nombre de la firma esperada; lista ORDENADA de nombres de
         parámetros (posicionales, *args, **kwargs, keyword-only) de la impl vs la esperada.
desambiguación: si target_line se da, verifica la def de `fn_name` en ESA línea; si no, la primera.
IGNORA: anotaciones de tipo, valores por defecto y el tipo de retorno (solo nombres + orden + forma).
casos: función ausente -> NO vacío; nombre distinto -> NO vacío; nº/orden/nombre de params distinto
       -> NO vacío; *args/**kwargs presente en una y no en la otra -> NO vacío.
...
- `signature_mismatch("def f(x): ...", "f", "def g(x)")` -> no vacío (nombre de la función)
- homónimas: `signature_mismatch("def f(a):...\n\ndef f(x):...", "f", "def f(x)", target_line=4)` -> `""`
